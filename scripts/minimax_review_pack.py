...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create one paste-ready review pack for MiniMax web from live KnowNet MCP calls.",
    )
    ...
    path = write_pack("minimax", args.url, args.output, args.ai_state_limit, args.copy)
    print(f"Wrote MiniMax review pack: {path}")
    if args.copy:
        print("Copied MiniMax review pack to clipboard.")
    return 0
