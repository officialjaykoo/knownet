...
| OpenAPI / JSON Schema | schema reference, reusable component thinking, validation errors | `schema_ref`, `contract_ref`, `validate_packet_schema_core` |
...
The concrete schema lives at `docs/schemas/packet.p26.v1.schema.json` and uses
`$id: /api/schemas/packet/p26.v1`.

## Mapping Rules

- Use `protocol_version` for protocol-level versioning and parser behavior.
- Use `schema_ref` and `contract_ref` for contract/schema identity.
...
- Use MCP standard method names (`resources/list`, `resources/read`,
  `tools/list`, `tools/call`) and namespaced `knownet.*` tool names.
