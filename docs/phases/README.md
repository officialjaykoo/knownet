# Phase Archive

These files are implementation history and acceptance criteria for completed
KnowNet phases. They are kept for traceability, but current operation should
start from the top-level README, runbook, security plan, model-run guide, and
release evidence.

Use phase files when you need to understand why a feature exists or which
completion gate introduced it.

Latest standardization phase:

```txt
PHASE_23_TASKS.md - Standard MCP baseline:
  standard KnowNet MCP resource URIs, proposal-only tools, prompt names, and
  tests that keep raw/admin surfaces out.
```
