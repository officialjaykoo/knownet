...
Implementation status: completed foundation with Phase 17 operator integration.
...
Done:
  ...
  - Phase 17 Operator Console surfaces model_review_runs
  - Phase 17 provider matrix distinguishes mocked/configured/live_verified
  - Phase 17 AI state quality gate checks model context readiness
  - Phase 17 release readiness includes provider verification summary

Not done in this step:
  - Qwen API runner
  - Manus live Custom MCP/API registration
  - direct browser restore execution with typed confirmation
  - full provider documentation matrix refresh
```

Phase 17 follow-up clarified the completion boundary:

```txt
Gemini API-key execution works through the server-side model runner when
GEMINI_RUNNER_ENABLED=true and GEMINI_API_KEY is configured.

Mocked runs never count as live_verified.
Configured credentials without a successful non-mock run count as configured.
A successful non-mock provider run that reaches dry_run_ready/imported counts
as live_verified.

Operator UI, AI state quality, provider verification, release readiness, and
restore-plan inspection belong to Phase 17, not Phase 16.
```
...
docs/RELEASE_EVIDENCE.md
  Release evidence is generated by scripts/release_check.ps1 after Phase 17.
```
...
7. Record a Manus setup attempt in docs/MCP_CLIENTS.md.
```
