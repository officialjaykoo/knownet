...
@router.post("/migrate")
async def migrate(request: Request, actor: Actor = Depends(require_admin_access)):
    started = time.perf_counter()
    phases = [
        {"phase": 1, "status": "already_applied"},
        {"phase": 2, "status": "already_applied"},
        {"phase": 3, "status": "already_applied"},
        {"phase": 4, "status": "already_applied"},
        {"phase": 5, "status": "already_applied"},
    ]
    backup = await _create_snapshot_archive(request, actor, prefix="pre-migrate")
    await ensure_phase6_schema(request.app.state.settings.sqlite_path)
    phases.append({"phase": 6, "status": "applied"})
    graph_rebuild = await request.app.state.rust_core.request(
        "rebuild_graph_for_vault",
        {
    ...
    await write_audit_event(
        ...
        target_type="schema",
        target_id="knownet",
        metadata={"phases": phases, "backup_created": backup["path"], "graph_rebuild": graph_rebuild},
    )
    return {
        "ok": True,
        "data": {
